...
- ⭐ `all_results`: ALL runs (including failed ones) for reproducibility
...
⭐ **NEW COLUMNS** for advanced statistical analysis:
...
⭐ **NEW FILES** generated by `generate_*_global_ranks()`:
...
### NEW: Fixed-Target ECDF Plots

⭐ **`rastrigin_ecdf_{config}.png`** — COCO/BBOB standard runtime-to-target
- Separate subplots for Gold, Silver, Bronze targets
...
### NEW: ERT (Expected Running Time) Plots

⭐ **`rastrigin_ert_{level}.png`** — Bar charts with CI
...
⭐ **NEW**: Standard COCO/BBOB metric for runtime-to-target.
...
⭐ **NEW for Rastrigin**: Integrated area under log convergence curve
...
⭐ **NEW**: Standard robustness metric
